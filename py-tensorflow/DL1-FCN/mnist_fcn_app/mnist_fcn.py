...
batch_size = st.sidebar.selectbox('batch_size를 선택하세요.', [100, 200, 300, 600], key="<uniquevalueofsomesort21>")
epochs = st.sidebar.selectbox('epochs횟수를 선택하세요.', [10, 20, 50, 100], key="<uniquevalueofsomesort22>")
loss_function = st.sidebar.selectbox('Loss function', [
                                     'mean_squared_error', 'mean_absolute_error', 'categorical_crossentropy'], key="<uniquevalueofsomesort23>")
optimizer = st.sidebar.selectbox('Optimizer', ['SGD', 'RMSprop', 'Adam'], key="<uniquevalueofsomesort24>")
...
# Flattening to make an input to FCN
...
if st.checkbox('첫번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb1'):
    fcn1 = st.selectbox(
        "노드의 갯수 :", [32, 64, 128, 256, 512], key="<uniquevalueofsomesort01>")
    act1 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort02>")
    ...
    if st.checkbox('첫번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb11'):
        drop1 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort03>")
...
if st.checkbox('두번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb2'):
    fcn2 = st.selectbox(
        "노드의 갯수 :", [32, 64, 128, 256, 512], key="<uniquevalueofsomesort25>")
    act2 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort26>")
    ...
    if st.checkbox('두번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb21'):
        drop2 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort27>")
...
if st.checkbox('세번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb3'):
    fcn3 = st.selectbox(
        "노드의 갯수 :", [32, 64, 128, 256, 512], key="<uniquevalueofsomesort31>")
    act3 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort32>")
    ...
    if st.checkbox('세번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb31'):
        drop3 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort33>")
...
if st.checkbox('네번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb4'):
    fcn4 = st.selectbox(
        "노드의 갯수 :", [32, 64, 128, 256, 512], key="<uniquevalueofsomesort41>")
    act4 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort42>")
...
# Summary of model
if st.checkbox('모델의 구조를 확인.'): #입력 빼면 4층구조 - 3개의 hidden layer + 1개의 output layer
    # st.write(model)
...
model_best = keras.models.load_model('./model/fcn3_best.hdf5') #keras 안 models 모듈 안 load_model 함수 사용
...
# Show random prediction results, and compare with best model
