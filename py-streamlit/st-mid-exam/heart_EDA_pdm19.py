...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm19')
...
with row2_2: 
    corr_y = st.selectbox("Correlation - Y variable", options=df.columns, index=df.columns.get_loc("thalach"))
...
figf = plt.figure(figsize=(5,4))
high_corr = ['age', 'cp', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal']
df_temp = df.copy()
df_temp['target'] = df_temp['target'].replace([0,1], ['noDM', 'DM'])
sns.heatmap(df[high_corr].corr(), annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(figf)