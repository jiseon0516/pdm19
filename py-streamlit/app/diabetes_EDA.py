...
classes=df.Outcome #결과
noDB,DB=classes.value_counts() #빈도
...
st.sidebar.write(fig0) #sidebar에 바차트 그림
# pi-chart
fig1=px.pie(df, values=df.Outcome.value_counts(), 
            width=350, height=300, 
            names=['noDM','DM'], color=['noDM','DM'], 
            color_discrete_map=dict(noDM = 'green', DM = 'red')) 
st.sidebar.write(fig1) #sidebar에 파이차트 그림
...
st.write(f'##### features = {list(df.columns)[:-1]}') #-이해
st.write(f'##### classes = {pd.unique(classes)}') #-이해
st.write('***')
...
row0_1, row0_space2, row0_2 = st.beta_columns(
    (1, .1, 1)) #1:1 10%유격 (반으로 나누는데 10%간격두고)
